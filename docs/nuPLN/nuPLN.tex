...
\item $\DataxD : \Omega \to \Bool$, a Boolean random variable indexed
  ...
  curried fashion
  $$(\anymu\ x) = \Datax$$
  Finally, $\DataxD$ can be viewed as a family of boolean random
  variables, or equivalently as a random function
  $$\Omega \to \Domain \to \Bool$$
  The order (whether $\Omega$ comes before or after $\Domain$) does
  not matter, however in practice we will find that it is easier to
  move $\Omega$ at the front and then forget about it, since the world
  we live in, $\omega$, remains the same through our entire subjective
  experience.
\end{itemize}
